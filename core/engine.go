...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	...
	if isResume {
		if validator, ok := agent.(AgentSessionWorkspaceValidator); ok {
			if !validator.SessionBelongsToCurrentWorkspace(startSessionID) {
				slog.Info("session doesn't belong to current workspace, starting fresh",
					"session_key", sessionKey,
					"agent_session", startSessionID,
				)
				session.SetAgentSessionID("", agent.Name())
				sessions.Save()
				startSessionID = ""
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	default:
		...
		// Not a cc-connect command — fall through to agent.
		return false
	}
	return true
}
