...
func TestProcessInteractiveEvents_DropsStopHookRejectedDraft(t *testing.T) {
	...
	e.processInteractiveEvents(context.Background(), state, session, e.sessions, sessionKey, "m-hook-rejected", time.Now(), nil, nil, state.replyCtx, 0)
...
func TestProcessInteractiveEvents_NoHookRejectionKeepsTextAcrossToolBoundary(t *testing.T) {
	...
	e.processInteractiveEvents(context.Background(), state, session, e.sessions, sessionKey, "m-no-hook-rejection", time.Now(), nil, nil, state.replyCtx, 0)
